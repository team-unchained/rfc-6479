...
이러한 에러 응답의 세부 사항은 이 명세의 범위를 벗어나지만, 본 문서는 OAuth 토큰 인증 방식 간에 공유되는 에러 값을 위한 공통 등록소(registry)를 11.4절에 정의합니다.

OAuth 토큰 인증을 위해 설계된 새로운 인증 방식은 클라이언트에게 에러 상태 코드를 제공하는 메커니즘을 정의해야 합니다 (SHOULD). 
허용되는 에러 값들은 이 명세서에 의해 설정된 에러 등록소에 등록되어야 합니다.
...
동일한 방식으로 에러 값을 등록소에 바인딩할 수 있습니다 (MAY).
